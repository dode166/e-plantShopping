...
// ⭐⭐ (تعديل 1) استيراد useSelector لقراءة حالة Redux
import { useDispatch, useSelector } from 'react-redux'; 
import { addItem } from './CartSlice.jsx';
import './ProductList.css';
import CartItem from './CartItem'; 

function ProductList({ onHomeClick, onCartClick, onPlantsClick, showCart, handleContinueShopping }) {
    
    const dispatch = useDispatch();

    // ⭐⭐ (تعديل 2) استخدام useSelector لقراءة حالة السلة
    // نقوم بحساب إجمالي عدد الكميات (quantity) لجميع العناصر
    const totalItems = useSelector((state) => 
        state.cart.items.reduce((total, item) => total + item.quantity, 0)
    );

    // حالة React: لتتبع المنتجات التي أضيفت بالفعل (لتعطيل الزر - 6 نقاط)
    const [addedToCart, setAddedToCart] = useState({});

    // ⚠️ قائمة النباتات (بدون تغيير عن الإصدار المصحح السابق)
    const plantsArray = [
        {
    ...
    // دالة الإضافة إلى السلة (التي ترسل الإجراء إلى Redux)
    const handleAddToCart = (product) => {
        dispatch(addItem(product)); 

        // تحديث حالة الواجهة لتعطيل زر المنتج (6 نقاط)
    ...
    const styleObjUl = {
        ...
        alignItems: 'center',
        width: '1100px',
        position: 'relative' // لتحديد موقع رقم العناصر
    }
    const styleA = {
        color: 'white',
        fontSize: '30px',
        ...
        <div>
            <div className="navbar" style={styleObj}>
                ...
                <div style={styleObjUl}>
                    <div> <a href="#" onClick={(e) => onPlantsClick(e)} style={styleA}>Plants</a></div>
                    <div> 
                        <a href="#" onClick={(e) => onCartClick(e)} style={styleA}>
                            <h1 className='cart'>
                        ...
                        {/* ⭐⭐ (تعديل 4) عرض عدد العناصر (7 نقاط) */}
                        {totalItems > 0 && <span style={cartCountStyle}>{totalItems}</span>}
                    </div>
                </div>
            </div>
            
            {/* المنطق الأساسي لعرض المنتجات */}
            {!showCart ? (
                            ...
                            <div className="product-list">
                                {category.plants.map((plant, plantIndex) => (
                                    <div className="product-card" key={plantIndex}>
                                        ...
                                        {/* عرض التكلفة باستخدام تنسيق العملة */}
                                        <div className="product-cost">${plant.cost.toFixed(2)}</div> 
                                        
                                        {/* زر "Add to Cart" (6 نقاط) */}
                                        <button
                                            className="product-button"
                                            onClick={() => handleAddToCart(plant)}
                                            // التعطيل إذا كان المنتج قد أضيف بالفعل
